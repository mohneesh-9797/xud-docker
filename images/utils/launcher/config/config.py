...
class Config:
    ...
    def update_bitcoind_kind(self, node, parsed):
        ...
        if hasattr(self.args, opt):
            ...
            if value not in ["native", "external", "neutrino"]:
                raise CommandLineArgumentValueError("Invalid value of option \"--{}\": {}".format(opt, value))
            node["mode"] = value
        ...
        if node["mode"] == "external":
            ...
            if hasattr(self.args, opt):
                ...
                try:
                    ...
                    raise NetworkConfigFileValueError("Invalid value of option \"--{}\": {}".format(opt, value))
